...
export default function App() {
  ...
  // Global listener to trigger Download APK / Mobile / Container modal from anywhere
  React.useEffect(() => {
    const handleOpenDownload = () => setIsDownloadAppModalOpen(true);
    window.addEventListener('open-download-app-modal', handleOpenDownload);
    return () => window.removeEventListener('open-download-app-modal', handleOpenDownload);
  }, []);
  ...
  const handleRecordAttendance = (classId: string, status: 'present' | 'late') => {
    ...
    if (existingEnrollment) {
      ...
      const newEnrollmentRecord: Enrollment = {
        ...
        classId: classId,
        enrolledAt: getSynchronizedDate().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }),
    ...
    // B. LOG ATTENDANCE LOG (With Deduplication protection & University Network Time Sync)
    const syncNow = getSynchronizedDate();
    const timeString = syncNow.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' });
    const todayISO = syncNow.toISOString().split('T')[0];
          ...
          <div className="flex-1 flex flex-col min-w-0 overflow-hidden h-full">
            ...
            <header className={`flex px-2.5 sm:px-6 py-2 sm:py-2.5 items-center justify-between gap-2 sm:gap-4 shrink-0 border-b border-zinc-200 dark:border-zinc-850 bg-white/95 dark:bg-zinc-950/95 backdrop-blur-xl text-zinc-900 dark:text-zinc-100 relative ${
              ...
              <CommandPalette
                ...
                accessibility={accessibility}
                onNavigate={(screenId) => {
                  handleSetScreen(screenId);
                }}
                isMobileBarVisible={isMobileBarVisible}
                setIsMobileBarVisible={setIsMobileBarVisible}
